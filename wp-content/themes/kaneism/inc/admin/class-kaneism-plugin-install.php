...
if ( ! class_exists( 'Kaneism_Plugin_Install' ) ) :
	/**
	 * The Kaneism plugin install class
	 */
	class Kaneism_Plugin_Install {
		...
		public function plugin_install_scripts( $hook_suffix ) {
			...
			wp_enqueue_script( 'kaneism-plugin-install', get_template_directory_uri() . '/assets/js/admin/plugin-install' . $suffix . '.js', array( 'jquery', 'updates' ), $kaneism_version, 'all' );

			wp_enqueue_style( 'kaneism-plugin-install', get_template_directory_uri() . '/assets/css/admin/plugin-install.css', array(), $kaneism_version, 'all' );
		}
			...
			if ( current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) {
				if ( is_plugin_active( $plugin_slug . '/' . $plugin_file ) ) {
					...
					$button = array(
						...
						'url'     => '#',
						'classes' => array( 'kaneism-button', 'disabled' ),
					);
				...
				</span> <?php echo /* translators: conjunction of two alternative options user can choose (in missing plugin admin notice). Example: "Activate WooCommerce or learn more" */ esc_html__( 'or', 'kaneism' ); ?>
...
return new Kaneism_Plugin_Install();
